...
The remaining second-order contribution pairs the three-graviton vertex with one matter
vertex,
\begin{equation}
S^{t}_{fi}=-\int d^{4}x_1\,d^{4}x_2\;
\bra{k_1,k_2\lambda_2}
T\bigl\{\mathcal{H}_{hhh}(x_1)\,\mathcal{H}_{h\varphi\varphi}(x_2)\bigr\}
\ket{k_3,k_4\lambda_4},
\end{equation}
the factor $2$ from $x_1\leftrightarrow x_2$ having cancelled the $1/2!$. Here the two
external gravitons attach to $\mathcal{H}_{hhh}$, the two external scalars to
$\mathcal{H}_{h\varphi\varphi}$, and the remaining $h$ are contracted into the graviton
propagator
\begin{equation}
\wick{\c h_{\alpha\beta}(x_1)\,\c h_{\rho\sigma}(x_2)}
=\int\frac{d^{4}q}{(2\pi)^{4}}\,
\frac{i\,P_{\alpha\beta,\rho\sigma}}{q^{2}+i\varepsilon}\,e^{-iq(x_1-x_2)},
\qquad
P_{\alpha\beta,\rho\sigma}=\tfrac12\bigl(\eta_{\alpha\rho}\eta_{\beta\sigma}
+\eta_{\alpha\sigma}\eta_{\beta\rho}-\eta_{\alpha\beta}\eta_{\rho\sigma}\bigr).
\end{equation}
Momentum conservation fixes
\begin{equation}
q=k_4-k_2=k_1-k_3,
\qquad
q^{2}=-2k_2\!\cdot\!k_4=-4\,\omega_2\omega_4\sin^{2}\frac{\theta}{2}.
\end{equation}


The two scalar vertex gives rise to the stress energy tensor:
\begin{equation}
T^{\rho\sigma}(k_1,k_3)=k_1^{\rho}k_3^{\sigma}+k_3^{\rho}k_1^{\sigma}
-\eta^{\rho\sigma}\bigl(k_1\!\cdot\!k_3-m^{2}\bigr),
\end{equation}
its contraction with the propagator tensor part gives:
\begin{equation}
P_{\alpha\beta,\rho\sigma}\,T^{\rho\sigma}
=T_{\alpha\beta}-\tfrac12\eta_{\alpha\beta}T^{\rho}{}_{\rho}
=k_{1\alpha}k_{3\beta}+k_{3\alpha}k_{1\beta}-\eta_{\alpha\beta}\,m^{2}.
\label{eq:PT}
\end{equation}
This is the mechanism that generates the newtonian force: at zero momentum transfer as 
we alredy saw in the previous chapters.

Now we have to contract this with the three graviton vortex:
