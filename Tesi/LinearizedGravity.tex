...
&\overset{\sim}{=}\ -2(E\omega)^2(\epsilon^{(2)}\!\cdot\epsilon^{(4)})(1-n_2\!\cdot\!n_4)
\end{align*}
(the omitted terms, denoted $\cdots$, are just repetitions of the terms above).
...
We will call the terms inside the $\{\}$ brakets $\mathcal{M}$.
...
\begin{equation}
    ...
    \frac{E^4}{sen^4 \theta/2} |\mathcal{M}|^2 d^3p_4
\end{equation}

\begin{equation}
    \approx (1+|v|)(32\pi G)^2 \frac{E^2}{16} \int d |p_4| d \Omega_4 \delta(|p_4|-|p_2|)\frac{|p_4|}{|p_2|} \frac{|\mathcal{M}|^2}{sen^4 \theta /2}
\end{equation}


\begin{equation}
    \approx (1+|v|)(32\pi G)^2 \frac{E^2}{16} \int d \Omega_4 \frac{|\mathcal{M}|^2}{sen^4 \theta /2}
\end{equation}

\begin{equation}
    \frac{d\sigma}{d \Omega_4} \approx (1+|v|)(32\pi G)^2 \frac{E^2}{16} \frac{|\mathcal{M}|^2}{sen^4 \theta /2}
\end{equation}



Last but not least, we are interested in calculating what's inside the $|\mathcal{M}|^2$ term up to leading order at $v$.
...
If the incoming graviton is directed like the scattered particle along the $z$ axis, then $\vec{v}\cdot \vec{n_4} = v\cos\theta$ and $\vec{v}\cdot \vec{n_2} = v$. On the other hand
 ...
 It's interesting because we can see that we resemble a Rutherford type scattering as
 we can notice from the denominator. Actually, all of the interaction term with 
 the self interacting graviton gives us Rutherford scattering, which
 we could think of such as a classical matter interaction between a gravitational wave
 ongoing and matter. Basically, it is a wave of classical potential going.

 From the other interaction terms, such as the seagull and the virtual scalar exchange, 
 we get a different type of scattering, which is more like a Thomson scattering.
 We can see that, this type of scattering comes at order $\mathcal{O}(v^4)$, in accord
 to the quadripole dependence of the gravitational source.
 % ====================================================================
